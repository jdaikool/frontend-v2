...
export function useSwapContract(poolName: PoolName): Swap | null {
  ...
  const yakSwapContract = useContract(
    chainId ? YAK_SWAP_CONTRACT_ADDRESSES[chainId] : undefined,
    SWAP_ABI,
    withSignerIfPossible,
  )
  ...
  return useMemo(() => {
    if (poolName === ZBTC_WBTC_POOL_NAME) {
      ...
      return yakSwapContract as Swap
    }
...
export function useAllContracts(): AllContractsObject | null {
  ...
  const yakContract = useTokenContract(YAK) as Erc20
  const myakContract = useTokenContract(mYAK) as Erc20
  ...
  const yakSwapTokenContract = useTokenContract(YAK_SWAP_TOKEN) as Swap
  ...
  return useMemo(() => {
    return {
      ...
      [mYAK.symbol]: myakContract,
      [YAK.symbol]: yakContract,
      ...
      [YAK_SWAP_TOKEN.symbol]: yakSwapTokenContract,
    }
  }, [
    wbtcContract,
    daiContract,
