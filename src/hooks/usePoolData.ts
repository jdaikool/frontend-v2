...
export default function usePoolData(
  ...
  const stakedTokenBalance = useStakedTokenBalance(POOL?.poolId | 0)
    ...
    async function getSwapData(): Promise<void> {
      ...
      const tokenBalances: BigNumber[] = await Promise.all(
        POOL.poolTokens.map(async (token, i) => {
          let balance = await swapContract?.getTokenBalance(i)
          if (token.symbol == "mYAK" && balance) {
            balance = balance.div(1000000)
          }
      ...
      const userPoolTokenBalancesUSDSum: BigNumber =
        userPoolTokenBalancesUSD.reduce((sum, b) => sum.add(b), Zero)

      /** For swap pool, staked Lp token also count towards userLpTokenBalance */
      const userLpTokenBalanceIncludeStaked =
        userLpTokenBalance.add(stakedTokenBalance)
      ...
      const userPoolTokenBalancesSumIncludeStaked: BigNumber =
        userPoolTokenBalancesIncludeStaked.reduce((sum, b) => sum.add(b), Zero)
      ...
      const userPoolTokenBalancesUSDSumIncludeStaked: BigNumber =
        userPoolTokenBalancesUSDIncludeStaked.reduce(
          (sum, b) => sum.add(b),
          Zero,
        )
