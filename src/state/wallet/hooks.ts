...
export function usePoolTokenBalances(): { [token: string]: BigNumber } | null {
  ...
  const yakTokenBalance = useTokenBalance(YAK)
  const myakTokenBalance = useTokenBalance(mYAK)
  ...
  const yakSwapTokenBalance = useTokenBalance(YAK_SWAP_TOKEN)
  const poolTokensBalances = useMemo(
    () => ({
      ...
      [YAK.symbol]: yakTokenBalance,
      [mYAK.symbol]: myakTokenBalance,
      ...
      [YAK_SWAP_TOKEN.symbol]: yakSwapTokenBalance,
    }),
    [
      daiTokenBalance,
      dusdtTokenBalance,
